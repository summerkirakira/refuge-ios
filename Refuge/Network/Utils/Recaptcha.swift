...
let ENDPOINT = "api2"
let PARAMS = "ar=1&k=6LerBOgUAAAAAKPg6vsAFPTN66Woz-jBClxdQU-o&co=aHR0cHM6Ly9yb2JlcnRzc3BhY2VpbmR1c3RyaWVzLmNvbTo0NDM.&hl=zh-CN&v=vP4jQKq0YJFzU6e21-BGy3GP&size=invisible&cb=lam2123evvtl"

let K = "6LerBOgUAAAAAKPg6vsAFPTN66Woz-jBClxdQU-o"
let CO = "aHR0cHM6Ly9yb2JlcnRzc3BhY2VpbmR1c3RyaWVzLmNvbTo0NDM."
let V = "vP4jQKq0YJFzU6e21-BGy3GP"


let BASE = "https://recaptcha.net/recaptcha/"
let CONTENT_TYPE = "application/x-www-form-urlencoded"

let firstStagePattern = "\"recaptcha-token\" value=\"(.*?)\""
let secondStagePattern = "\"rresp\",\"(.*?)\""


func getCapturedGroups(inputString: String, pattern: String) -> [String] {
    do {
        ...
        if let match = regex.firstMatch(in: inputString, options: [], range: NSRange(location: 0, length: inputString.utf16.count)) {
            ...
            for i in 1..<match.numberOfRanges {
                ...
                let capturedSubstring = (inputString as NSString).substring(with: range)
                capturedGroups.append(capturedSubstring)
            }
...
let recaptchaV3 = RecaptchaV3()

class RecaptchaV3 {
    func getPostData(token: String) -> [String: String] {
        return[
            "v": V,
            "reason": "q",
            "c": token,
    ...
    func getFirstStageToken() async -> String {
        var url = "\(BASE)\(ENDPOINT)/anchor?\(PARAMS)"
        var response = try? await simpleReq.requestAsync(endPoint: url)
        if response == nil {
            return ""
        }
        var pattern = try? getCapturedGroups(inputString: response!, pattern: firstStagePattern)
        if pattern == nil {
            return ""
        }
//        debugPrint(response)
    ...
    func getSecondStageToken(firstStageToken: String) async -> String {
        var url = "\(BASE)\(ENDPOINT)/reload?k=\(K)"
        var postData = getPostData(token: firstStageToken)
        let response = try? await simpleReq.postAsync(url: url, parameters: postData)
        if response == nil {
            return ""
        }
        var pattern = try? getCapturedGroups(inputString: response!, pattern: secondStagePattern)
        if pattern == nil {
            return ""
        }
//        debugPrint(response)
    ...
    func getRecaptchaToken() async -> String {
        var firstStageToken = await getFirstStageToken()
